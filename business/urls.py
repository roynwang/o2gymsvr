...
urlpatterns = patterns('',
		...
		#url(r'^api/c/(?P<pk>[0-9]+)/$', CourseItem.as_view()),
                url(r'^api/homework/(?P<pk>[0-9]+)/$', HomeworkItemView.as_view()),
		...
		url(r'^api/finance/(?P<pk>[0-9]{0,8})/$', FinanceItem.as_view()),
		...
		url(r'^api/(?P<name>[a-zA-Z0-9]{4,64})/reimbursement/$', ReimbursementList.as_view()),
		...
		url(r'^api/g/(?P<pk>[0-9]+)/groupcourseinstance/(?P<date>[0-9]{8})/week/$', GymGroupCourseWeekList.as_view()),
		...
		url(r'^api/(?P<name>[a-zA-Z0-9]{4,64})/kpidetail/(?P<date>[0-9-]{10})/$',CustomerWeeklyKPIItemView.as_view()),
		...
		#body eval
                ...
                url(r'^api/(?P<name>[a-zA-Z0-9]{4,20})/homework/$', HomeworkListView.as_view()),
		...
		url(r'^api/a/(?P<pk>[0-9]+)/$', GymMap.as_view()),
                ...
                url(r'^api/gs/(?P<courseid>[0-9]+)/$',show_post_survey_group),
                url(r'^api/cs/(?P<courseid>[0-9]+)/survey/$',ScheduleSurvey.as_view()),
                url(r'^api/g/(?P<gymid>[0-9]+)/survey/$',GymSurveyList.as_view()),
                url(r'^api/cs/(?P<courseid>[0-9]+)/complete/$',show_complete_survey),
                url(r'^api/gs/(?P<courseid>[0-9]+)/complete/$',show_complete_survey_group),
